...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if (empty($input_salary)){
        $salary_err="Enter the salary amount.";
    }elseif (!ctype_digit($input_salary)){
        $salary_err ="Enter a positive integer value";
    }else{
        $salary = $input_salary;
    }
    if (empty($name_err)&& empty($address_err) && empty($salary_err)){
        $sql = "INSERT INTO employee (name, address, salary)VALUE (?,?,?)";
...
<div class="wrapper">
    <div class="container_fluid">
        <div class="row">
            ...
            <p>Please fill this and submit to add employee record the database</p>
